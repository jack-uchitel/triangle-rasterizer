...
void mesh3DTrueNormal(
        const GLdouble a[], const GLdouble b[], const GLdouble c[],
        GLdouble normal[3]) {
...
void mesh3DFlatNormals(meshMesh *mesh, GLuint n) {
    GLuint i, *tri;
...
void mesh3DSmoothNormals(meshMesh *mesh, GLuint n) {
    GLuint i, *tri;
...
GLuint mesh3DInitializeBox(
        meshMesh *mesh, GLdouble left, GLdouble right, GLdouble bottom,
        GLdouble top, GLdouble base, GLdouble lid) {
...
the input. */
void mesh3DRotateVector(GLdouble theta, const GLdouble v[2], GLdouble vRot[2]) {
...
finished. */
GLuint mesh3DInitializeRevolution(
        meshMesh *mesh, GLuint zNum, const GLdouble z[], const GLdouble r[],
        const GLdouble t[], GLuint sideNum) {
    GLuint i, j, error;
...
GLuint mesh3DInitializeSphere(
        meshMesh *mesh, GLdouble r, GLuint layerNum, GLuint sideNum) {
    GLuint error, i;
...
when finished. */
GLuint mesh3DInitializeCapsule(
        meshMesh *mesh, GLdouble r, GLdouble l, GLuint layerNum,
        GLuint sideNum) {
    GLuint error, i;
    GLdouble theta;
...
are smooth except where the side meets the ends. Don't forget to meshDestroy
when finished. */
GLuint mesh3DInitializeCylinder(
        meshMesh *mesh, GLdouble r, GLdouble l, GLuint sideNum) {
    GLuint triNum = 4 * sideNum;
    GLuint vertNum = 4 * sideNum + 4;
    GLuint error = meshInitialize(mesh, triNum, vertNum, 3 + 2 + 3);
    if (error != 0)
        return error;
    GLdouble fraction, attr[3 + 2 + 3];
    /* Make the 2 * sideNum + 2 side vertices. */
    attr[7] = 0.0;
    for (GLuint i = 0; i < sideNum; i += 1) {
        fraction = (GLdouble)i / sideNum;
    ...
    for (GLuint i = 0; i < sideNum; i += 1) {
        attr[0] = r * cos(2.0 * M_PI * (GLdouble)i / sideNum);
        attr[1] = r * sin(2.0 * M_PI * (GLdouble)i / sideNum);
    ...
    for (GLuint i = 0; i < sideNum - 1; i += 1)
...
GLdouble zs[3][4] = {
    ...
    {6.0, 5.0, 3.0, 1.0},
    {4.0, 3.0, -1.0, -2.0}};
GLuint error = mesh3DInitializeLandscape(&mesh, 3, 4, 20.0, (GLdouble *)zs); */
GLuint mesh3DInitializeLandscape(
        meshMesh *mesh, GLuint size, GLdouble spacing, const GLdouble *data) {
    GLuint i, j, error;
    GLuint a, b, c, d;
...
/* Given a landscape, such as that built by mesh3DInitializeLandscape. Builds a
...
GLuint mesh3DInitializeDissectedLandscape(
        meshMesh *mesh, const meshMesh *land, GLdouble angle,
        GLuint noMoreThan) {
